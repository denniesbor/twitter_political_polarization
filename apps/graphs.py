...
def chart_type_plot(chart_type, df, template):
    ...
    if chart_type == 'Histogram':
        ...
        df = df[['compound_sentiment','party','tweet_id']].groupby(['compound_sentiment','party']).count()
        df = df.reset_index()
        fig = px.scatter(df, x='compound_sentiment',
                            y='tweet_id',
                            labels={
...
def clean_tweets(tweet:str):
    ...
    tweet_list = [word for word in re.findall(r'\w+', tweet) if word not in stopwords_set]
...
def get_wc_words(df,max_word_count=70):
  ...
  all_words = [item for sublist in [word for word in list(df.text)] for item in sublist]
...
def tree_map(common_words, template):
    ...
    fig = px.treemap(pd.DataFrame(common_words, columns=["common words","values"]),
                     path=['common words'],
                     values='values', 
                     template=template,
                     height=250)
